...
Recall that the GOE is the collection of symmetric matrices with Gaussian entries. To simplify our calculations a bit, let's consider matrices in $\mathbb{R}^{2\times 2}$ such that the diagonal entries are distributed as $N(0,1)$ and the off-diagonals as $N(0,\frac{1}{2})$. In other words:

$$\begin{bmatrix} x_1 & x_2 \\ x_3 & x _4 \end{bmatrix} , \; x_1,x_4 \sim N(0,1), x_2, x_3 \sim N(0,\frac{1}{2})$$
